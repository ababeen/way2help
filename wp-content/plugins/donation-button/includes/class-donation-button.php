...
class Donation_Button {
    ...
    public function __construct() {
        ...
        add_filter("{$prefix}plugin_action_links_" . DBP_PLUGIN_BASENAME, array($this, 'donation_button_plugin_action_links'), 10, 4);
        add_filter('widget_text', 'do_shortcode');
    }

    public function donation_button_plugin_action_links($actions, $plugin_file, $plugin_data, $context) {
        $custom_actions = array(
            'configure' => sprintf('<a href="%s">%s</a>', admin_url('options-general.php?page=donation-button'), __('Configure', 'donation-button')),
            'docs' => sprintf('<a href="%s" target="_blank">%s</a>', 'https://www.premiumdev.com/product/paypal-donation-buttons/', __('Docs', 'donation-button')),
